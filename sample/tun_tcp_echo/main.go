...
func serverListen(listenEP *global.Endpoint, s *stack.Stack, server string, port uint16) error {
...
func clientConnect(clientEP *global.Endpoint, s *stack.Stack, server string, port uint16) error {
	ipport := fmt.Sprintf("%v:%v", server, port)
	udpAddr, err := net.ResolveUDPAddr("udp", ipport)
	if err != nil {
		log.Errorf("Invalid port: %v", err)
