...
const (
	fakeNetNumber    global.NetworkProtocolNumber = math.MaxUint32
	fakeNetHeaderLen                              = 12
...
func (f *fakeNetworkEndpoint) NICID() global.NICID {
	return f.nicid
}
...
func (f *fakeNetworkEndpoint) HandlePacket(r *stack.Route, vv *buffer.VectorisedView) {
	...
	f.dispatcher.DeliverTransportPacket(r, global.TransportProtocolNumber(b[2]), vv)
}
...
func (f *fakeNetworkEndpoint) PseudoHeaderChecksum(protocol global.TransportProtocolNumber, dstAddr global.Address) uint16 {
	return 0
}

func (f *fakeNetworkEndpoint) WritePacket(r *stack.Route, hdr *buffer.Prependable, payload buffer.View, protocol global.TransportProtocolNumber) error {
...
func (f *fakeNetworkProtocol) Number() global.NetworkProtocolNumber {
...
func (*fakeNetworkProtocol) ParseAddresses(v buffer.View) (src, dst global.Address) {
	return global.Address(v[1:2]), global.Address(v[0:1])
}

func (f *fakeNetworkProtocol) NewEndpoint(nicid global.NICID, addr global.Address, dispatcher stack.TransportDispatcher, linkEP stack.LinkEndpoint) (stack.NetworkEndpoint, error) {
...
func sendTo(t *testing.T, s *stack.Stack, addr global.Address) {
...
func testRoute(t *testing.T, s *stack.Stack, nic global.NICID, srcAddr, dstAddr, expectedSrcAddr global.Address) {
...
func testNoRoute(t *testing.T, s *stack.Stack, nic global.NICID, srcAddr, dstAddr global.Address) {
	_, err := s.FindRoute(nic, srcAddr, dstAddr, fakeNetNumber)
	if err != global.ErrNoRoute {
		t.Fatalf("FindRoute returned unexpected error, expected global.ErrNoRoute, got %v", err)
	}
}
...
func TestAddressRemoval(t *testing.T) {
	...
	if err := s.RemoveAddress(1, "\x01"); err != global.ErrBadLocalAddress {
		t.Fatalf("RemoveAddress failed: %v", err)
	}
}
...
func TestPromiscuousMode(t *testing.T) {
	...
	if err != global.ErrNoRoute {
		t.Fatalf("FindRoute returned unexpected status: expected %v, got %v", global.ErrNoRoute, err)
	}
...
func TestSubnetAcceptsMatchingPacket(t *testing.T) {
	...
	buf[0] = 1
	fakeNet.packetCount[1] = 0
	subnet, err := global.NewSubnet(global.Address("\x00"), global.AddressMask("\xF0"))
	if err != nil {
...
func TestSubnetRejectsNonmatchingPacket(t *testing.T) {
	...
	buf[0] = 1
	fakeNet.packetCount[1] = 0
	subnet, err := global.NewSubnet(global.Address("\x10"), global.AddressMask("\xF0"))
	if err != nil {
		t.Fatalf("NewSubnet failed: %v", err)
	}
	if err := s.AddSubnet(1, fakeNetNumber, subnet); err != nil {
