...
func newListenContext(stack *stack.Stack, rcvWnd seqnum.Size, v6only bool, netProto global.NetworkProtocolNumber) *listenContext {
	l := &listenContext{
		stack:    stack,
		rcvWnd:   rcvWnd,
		hasher:   sha1.New(),
...
func (l *listenContext) createConnectedEndpoint(s *segment, iss seqnum.Value, irs seqnum.Value, mss uint16) (*endpoint, error) {
	...
	n.effectiveNetProtos = []global.NetworkProtocolNumber{s.route.NetProto}
